...
class TestNamespace(unittest.TestCase):
    def test_decode_failure(self):
        ...
        def test_namespace1_invalid(self):
        ...
        def test_both_valid_ignored(self):
            for test in TEST_BOTH_NAMESPACES_VALID_IGNORED:
                ...
                parsed_type_names = set(p['tlv_type_name'] for p in
                                        parsed.values())
                self.assertTrue(parsed_type_names.issubset({"unknown"}))
                n2 = TestNamespace2()
                parsed, err = n2.parse(test_bytes)
                #print("parsed: %s err: %s" % (parsed, err))
                self.assertEqual(err, None)
        ...
        def test_namespace1_valid(self):
            ...
            for test in TEST_NAMESPACE1_VALID:
                ...
                #print("parsed: %s err: %s" % (parsed, err))
                self.assertEqual(err, None)
                self.assertTrue(json_cmp(parsed, test['expected']))
            print("done testing namespace1 valid")

        def test_namespace1_decoding_failure(self):
